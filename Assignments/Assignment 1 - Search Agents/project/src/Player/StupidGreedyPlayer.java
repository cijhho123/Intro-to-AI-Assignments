...
public class StupidGreedyPlayer extends Player {
    ...
    public StupidGreedyPlayer(StupidGreedyPlayer other){
        super(other.graph, other.location, other.id, Player.IS_DELIVERING_PACKAGES, Player.IS_NOT_SEARCH_AGENT); 
    }

    public StupidGreedyPlayer(StupidGreedyPlayer other, Graph g){
        super(other, g);
    }
    
    @Override
    public Player clone() {
        StupidGreedyPlayer p = new StupidGreedyPlayer(this);
        return p;
    }

    @Override
    public Player clone(Graph g) {
        StupidGreedyPlayer p = new StupidGreedyPlayer(this, g);
        return p;
    }

    //might introduce versioning to the graph, to compute new paths only when necessary
    @Override
    int getNextStep() {
        if(this.isHoldingPackages()){
            ...
            for(int packageIndex = 0; packageIndex < packageList.size(); packageIndex++){
                int[][] currData = this.getGraph().getCheapestPath(this.getLocation().getIndex(), 
                    packageList.get(packageIndex).getDestinationLocationIndex());
